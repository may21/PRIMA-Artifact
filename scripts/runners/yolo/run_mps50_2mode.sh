...
#
# Usage:
#   ./run_mps50_2mode.sh       # default: 3 repeats per mode
#   ./run_mps50_2mode.sh 3
...
read -r -a MODES <<< "${MODE_LIST:-mps50_unlimited mps50_memlimit}"
...
if [[ -n "${RESULT_ROOT_OVERRIDE:-}" ]]; then
  ...
  RUN_INDEX=1
  while [[ -e "${RESULT_BASE}/mps50_partitioning_${RUN_INDEX}" ]]; do
    RUN_INDEX=$((RUN_INDEX + 1))
  done
  RESULT_ROOT="${RESULT_BASE}/mps50_partitioning_${RUN_INDEX}"
fi
...
is_mps_mode() {
  [[ "$1" == mps*_unlimited || "$1" == mps*_memlimit ]]
}

is_memlimit_mode() {
  [[ "$1" == "ts_memlimit" || "$1" == mps*_memlimit ]]
}
...
{
  echo "experiment=concurrent_mps50_2mode"
  ...
  echo "modes=${MODES[*]}"
  echo "mps_active_thread_percentage=${MPS_ACTIVE_THREAD_PERCENTAGE:-50}"
  echo "effective_mps_sm=4-equivalent (CUDA_MPS_ACTIVE_THREAD_PERCENTAGE=${MPS_ACTIVE_THREAD_PERCENTAGE:-50})"
    ...
    for workload in "${WORKLOADS[@]}"; do
      ...
      if is_mps_mode "$mode"; then
        create_args+=(
          ...
          -e CUDA_MPS_ACTIVE_THREAD_PERCENTAGE="${MPS_ACTIVE_THREAD_PERCENTAGE:-50}"
        )
      fi
...
swap_max=0
mps_active_thread_percentage=$([[ "$mode" == mps* ]] && echo "${MPS_ACTIVE_THREAD_PERCENTAGE:-50}" || echo unset)
effective_sm=$([[ "$mode" == mps* ]] && echo "4-equivalent" || echo unrestricted)
